...
export default function Navbar() {
    ...
    const navLinks = (
        <>
            <Link href="/" className={`md:px-2 lg:px-3 py-2 font-medium flex items-center gap-2 ${pathname === "/" ? "text-blue-600 font-semibold" : "text-gray-700 hover:text-gray-900"}`}
                onClick={() => setMobileMenuOpen(false)} >
                <FaHome className="text-lg" /> Home
            </Link>

            <Link href="/services" className={`md:px-2 lg:px-3 py-2 font-medium flex items-center gap-2 ${pathname === "/services" ? "text-blue-600 font-semibold" : "text-gray-700 hover:text-gray-900"}`}
                onClick={() => setMobileMenuOpen(false)} >
            ...
            <Link href="/contact" className={`md:px-2 lg:px-3 py-2 font-medium flex items-center gap-2 ${pathname === "/contact" ? "text-blue-600 font-semibold" : "text-gray-700 hover:text-gray-900"}`}
                onClick={() => setMobileMenuOpen(false)} >
    ...
    return (
        <nav className={`fixed w-full top-0 z-50 bg-white border-b border-gray-200 transition-colors duration-300 ${scrolled ? "shadow-md" : ""}`} >
            <div className="max-w-7xl mx-auto flex items-center justify-between h-16 px-4">
                ...
                <div className="flex items-center">
                    <Link href="/" className="text-xl font-bold flex items-center">
                        <span className="text-2xl font-extrabold tracking-tight ml-2 block md:hidden lg:block text-transparent bg-clip-text bg-linear-to-r from-blue-500 to-yellow-500">
                            RepairPoint
                        </span>
                    </Link>
                </div>
                ...
                <div className="hidden md:flex items-center space-x-2 lg:space-x-3 z-105">
                    {!session ? (
                        <>
                            <Link href="/login" className="px-4 py-1 rounded-md text-sm border border-gray-200 hover:bg-gray-100" >
                                Sign In
                            </Link>
                            <Link href="/register" className="px-4 py-1 rounded-md text-sm bg-gray-900 text-white hover:bg-gray-700" >
                                Sign Up
                            </Link>
                        </>
                    ) : (
                        <div className="relative group cursor-pointer">
                            {session.user?.image ? (
                                <Image src={session?.user?.image} alt={session.user.name} width={48} height={48} className=" p-1 rounded-full border-2 border-gray-200" />
                            ) : (
                            ...
                            <div className="absolute right-2 border border-gray-200 top-12 w-45 bg-white rounded-md shadow origin-top scale-y-0 group-hover:scale-y-100 transition-transform duration-200 ease-out transform z-50">
                                ...
                                <button onClick={handleSignOut} className="flex items-center gap-2 px-4 py-2 w-full text-left hover:bg-gray-100">
                                    <FaSignOutAlt /> Logout
                                </button>
                            </div>
                        </div>
                ...
                <div className="flex md:hidden items-center gap-2">
                    <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="transition-all duration-300 ease-in-out transform text-xl" aria-label="Open menu" >
                        {mobileMenuOpen ? (
            ...
            {/* Mobile Nav */}
            <div className={`md:hidden absolute right-0 top-14 w-full z-50 transition-all duration-300 ease-in-out ${mobileMenuOpen ? "opacity-100 scale-y-100 max-h-[500px] pointer-events-auto" : "opacity-0 scale-y-95 max-h-0 pointer-events-none"} origin-top`}>
                <div className="border border-gray-200 w-11/12 mx-auto bg-white rounded-b-md shadow mt-2 p-4">
                    ...
                    {!session ? (
                        <div className="flex flex-col space-y-2 mt-2 px-3">
                            <Link href="/login" className="px-4 py-1 rounded-md text-sm border border-gray-200 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)} >
                                Sign In
                            </Link>
                            <Link href="/register" className="px-4 py-1 rounded-md text-sm bg-gray-900 text-white hover:bg-gray-700"
                                onClick={() => setMobileMenuOpen(false)} >
                        ...
                        <div className="border-t border-gray-200 pt-5 px-3 space-y-3 mt-2">
                            <Link href="/dashboard/profile" className="flex items-center gap-2 w-full" onClick={() => setMobileMenuOpen(false)} >
                                {session.user?.image ? (
                                    <Image src={session.user.image} alt={session.user.name || "User"} width={40} height={40} className="border border-gray-200 p-1 rounded-full" />
                                ) : (
